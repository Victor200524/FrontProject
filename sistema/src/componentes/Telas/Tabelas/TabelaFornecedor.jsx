...
export default function TabelaFornecedor(props){
    function excluirFornecedor(fornecedor){
        if(window.confirm("Deseja realmente excluir o fornecedor " + fornecedor.nome)){
            props.setListaDeFornecedor(props.listaDeFornecedor.filter((item)=>{
        ...
        <>
            <Container>
                <Button className="mb-3" variant="primary" onClick={()=>{
                    ...
                    props.setFornecedorSelecionado(false);
                }}> Adicionar 
                </Button>
                            ...
                            props.listaDeFornecedor?.map((fornecedor)=>{
                                return( 
                                    <tr>
                                        ...
                                        <td>
                                            <Button variant="warning" onClick={()=>{alterarFornecedor(fornecedor);}} > <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil-square" viewBox="0 0 16 16">
                ...
                <p> Quantidade de fornecedor(es) cadastrados: {props.listaDeFornecedor.length}</p>
            </Container>
        </>
    );
}